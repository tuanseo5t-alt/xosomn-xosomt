...
3. Điền thông tin:
   - **Repository name**: `xosomn-xosomt` (hoặc tên khác bạn muốn)
...
> **Lý do bước này quan trọng**: Nếu chưa upload code, tab Actions chỉ hiển thị trang "Get started with GitHub Actions" và không có menu nào ở bên trái. Các workflow chỉ xuất hiện SAU KHI bạn đã tạo file `.github/workflows/` trong repo.

### Cách tạo từng file trực tiếp trên GitHub (không cần cài gì):

Bạn cần tạo **13 file** theo thứ tự sau. Mỗi file làm theo các bước giống nhau:

**Các bước tạo một file:**
1. Vào repository của bạn trên GitHub (ví dụ: `github.com/tuanseo5t-alt/api-xosomn-xosomt`)
2. Nhấn **"Add file"** → **"Create new file"**
3. Ở ô **"Name your file..."** gõ đường dẫn file (xem bảng bên dưới)
4. Copy nội dung file từ thư mục `xosomn-xosomt/` và dán vào ô lớn bên dưới
5. Kéo xuống cuối trang → nhấn nút xanh **"Commit new file"**
6. Lặp lại cho file tiếp theo

> **Mẹo tạo thư mục con**: Khi gõ đường dẫn, gõ tên thư mục rồi nhấn **"/"** thì GitHub tự hiểu là thư mục con.
> Ví dụ: gõ `.github` → gõ `/` → gõ `workflows` → gõ `/` → gõ `backfill-xsmn.yml`

---

### Danh sách 13 file cần tạo (theo thứ tự):

| # | Gõ vào ô "Name your file..." | Copy nội dung từ file |
|---|---|---|
| 1 | `requirements.txt` | `xosomn-xosomt/requirements.txt` |
| 2 | `src/fetcher_xsmn.py` | `xosomn-xosomt/src/fetcher_xsmn.py` |
| 3 | `src/fetcher_xsmt.py` | `xosomn-xosomt/src/fetcher_xsmt.py` |
| 4 | `src/storage.py` | `xosomn-xosomt/src/storage.py` |
| 5 | `src/analyzer.py` | `xosomn-xosomt/src/analyzer.py` |
| 6 | `src/main_xsmn.py` | `xosomn-xosomt/src/main_xsmn.py` |
| 7 | `src/main_xsmt.py` | `xosomn-xosomt/src/main_xsmt.py` |
| 8 | `.github/workflows/fetch-xsmn.yml` | `xosomn-xosomt/.github/workflows/fetch-xsmn.yml` |
| 9 | `.github/workflows/fetch-xsmt.yml` | `xosomn-xosomt/.github/workflows/fetch-xsmt.yml` |
| 10 | `.github/workflows/backfill-xsmn.yml` | `xosomn-xosomt/.github/workflows/backfill-xsmn.yml` |
| 11 | `.github/workflows/backfill-xsmt.yml` | `xosomn-xosomt/.github/workflows/backfill-xsmt.yml` |
| 12 | `data/xsmn/.gitkeep` | (để trống, không cần nội dung) |
| 13 | `data/xsmt/.gitkeep` | (để trống, không cần nội dung) |

> **File 12 và 13**: Chỉ cần tạo file trống, không cần dán gì vào. Mục đích là giữ thư mục `data/` trong repo để GitHub Actions có chỗ lưu dữ liệu sau.

---

### Cách kiểm tra đã upload đúng chưa:

Sau khi tạo đủ 13 file, vào tab **"Actions"**. Nếu thấy menu bên trái có các mục:
- Daily Fetch XSMN
- Daily Fetch XSMT
- Backfill Historical XSMN Data
- Backfill Historical XSMT Data

→ **Bạn đã upload đúng!** Tiếp tục bước 4.

Nếu vẫn thấy trang "Get started" → Kiểm tra lại xem file `.github/workflows/backfill-xsmn.yml` đã được tạo chưa (đây là file quan trọng nhất cần có trước).

---

### Cách thay thế - Dùng GitHub Desktop (nếu có máy tính):
...
5. Copy **toàn bộ nội dung** trong thư mục `xosomn-xosomt/` vào thư mục vừa clone
...
> **Điều kiện**: Phải hoàn thành Bước 3 trước. Tab Actions phải hiển thị menu bên trái với tên các workflow.

### Bật quyền ghi cho GitHub Actions (làm một lần):

Trước khi chạy, cần bật quyền để Actions có thể tự lưu dữ liệu lên repo:

1. Vào **Settings** (tab ở trên cùng của repository)
2. Chọn **Actions** → **General** (menu bên trái)
...
1. Nhấn tab **"Actions"**
2. Ở menu bên trái, nhấn vào **"Backfill Historical XSMN Data"**
3. Bên phải xuất hiện nút **"Run workflow"** → nhấn vào đó
4. Một bảng nhỏ hiện ra, điền:
   - **from_date**: `2008-01-01`
   - **to_date**: để trống
   - **delay**: `2.0`
   - **no_analyze**: `true`
5. Nhấn nút **"Run workflow"** màu xanh lá

### Lấy dữ liệu XSMT (từ 27/01/2009):

Làm tương tự nhưng nhấn vào **"Backfill Historical XSMT Data"**:
...
```bash
cd đường/dẫn/đến/thư/mục/xosomn-xosomt
```
...
## 12. Đẩy Dữ Liệu Từ VSCode Lên GitHub

Sau khi chạy script trên máy và có dữ liệu trong thư mục `data/`, làm theo các bước sau để đưa lên GitHub.

### Bước 1: Cài Git (Chỉ Làm 1 Lần)

Tải Git tại: **https://git-scm.com/download/win**

Cài đặt → nhấn **Next** liên tục → nhấn **Finish**.

Sau khi cài xong, **đóng VSCode rồi mở lại**.
...
### Bước 2: Tạo Repo Trên GitHub (Chỉ Làm 1 Lần)

1. Vào **https://github.com/new**
2. **Repository name**: `xosomn-xosomt`
3. Chọn **Public**
...
### Bước 3: Kết Nối VSCode Với GitHub (Chỉ Làm 1 Lần)

Mở terminal trong VSCode (**Ctrl + `**), chạy lần lượt:

```bash
git init
git remote add origin https://github.com/tuanseo5t-alt/xosomn-xosomt.git
git add .
git commit -m "first commit"
git push -u origin main
```

Trình duyệt tự mở để đăng nhập GitHub → nhấn **Authorize** → quay lại terminal.

---

### Bước 4: Lần Sau Chỉ Cần 3 Lệnh

Mỗi khi chạy xong và muốn đẩy dữ liệu mới lên:

```bash
git add .
git commit -m "data: cap nhat du lieu"
...
### Đẩy Nhanh Bằng 1 Lệnh

Tạo shortcut bằng cách dán lệnh này vào terminal:

```bash
git add . && git commit -m "data: cap nhat $(date +'%Y-%m-%d')" && git pull --rebase origin main && git push
```

---

### Kiểm Tra Dữ Liệu Đã Lên GitHub Chưa

Sau khi push thành công, vào:
```
https://github.com/tuanseo5t-alt/xosomn-xosomt/tree/main/data
```
Nếu thấy thư mục `xsmn/` và `xsmt/` có file CSV → thành công!
